...
static void my_application_activate(GApplication* application) {
  ...
  // The Flutter layer draws its own title bar and rounded corners, so drop
  // the native decoration entirely. Going through gtk_window_set_titlebar
  // would keep GTK's client-side decoration frame (shadow/border/background)
  // around the transparent rounded UI; set_decorated(FALSE) removes all of
  // it and leaves a plain Flutter surface.
  gtk_window_set_decorated(window, FALSE);
  gtk_window_set_title(window, "CLIPER");

  apply_window_icon(window);

  // The Flutter UI draws its own rounded corners; enable an RGBA visual so
  // the window itself can be transparent and the desktop shows through
  // outside the rounded region (no square black backing).
  ...
  // Transparent so the rounded-corner Flutter UI is not backed by an opaque
  // square (see the RGBA visual setup above).
