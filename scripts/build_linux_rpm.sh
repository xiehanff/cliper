...
tar -C "$(dirname "$bundle_dir")" \
  -czf "$rpm_top/SOURCES/cliper-bundle.tar.gz" bundle
tar -C "$repo_root/linux/icons" \
  -czf "$rpm_top/SOURCES/cliper-icons.tar.gz" hicolor
cp "$repo_root/linux/com.cliper.app.desktop" \
  "$rpm_top/SOURCES/com.cliper.app.desktop"
...
rpmbuild \
  ...
  -bb "$repo_root/packaging/cliper.spec"

rpm_artifact="$(find "$rpm_top/RPMS" -type f \
  -name "cliper-${app_version}-${app_release}.*.rpm" -print -quit)"
