...
app_bundle="$repo_root/build/macos/Build/Products/Release/CLIPER.app"
release_dir="$repo_root/build/macos/release"
dmg_path="$release_dir/cliper-${app_version}.dmg"
...
mkdir -p "$release_dir"
stage_dir="$(mktemp -d "${TMPDIR:-/tmp}/cliper-dmg.XXXXXX")"
cleanup() {
  rm -rf "$stage_dir"
}
trap cleanup EXIT

ditto "$app_bundle" "$stage_dir/CLIPER.app"
